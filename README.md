...
Wiped Clean for WEBpt6 7-20-2019 .
...
- [ ] 
Describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do?
        Actions are information that send data from the application to the store. sent using dispatch actions are the only source of information for the store.
        Reducers determine how state changes in response to actions sent to the store. Actions describe what happened Reducers  determine a change in state.
        Store holds state in all aspects meaning it allows access to state and allows it to be updated. 
Why is the store known as a 'single source of truth' in a redux application?
        because the store holds the state, im not sure.
- [ ] 
What is the difference between Application state and Component state? 
        Application state is global and componenet state is local.
When would be a good time to use one over the other?
        if component state will change then you should use that im not sure what you are asking
- [ ] 
Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
        Redux thunk is middleware, it allows you to write action creators that return a function instead of an action 
